...
module internal WebPInterop =
    ...
    extern int WebPConfigInitInternal(WebPConfig& config, int preset, float32 quality, int abiVersion)
    ...
    extern nativeint WebPAnimEncoderNewInternal(int width, int height, WebPAnimEncoderOptions& options, int abiVersion)

    [<DllImport("libwebpmux", EntryPoint = "WebPAnimEncoderAdd", CallingConvention = CallingConvention.Cdecl)>]
    extern int WebPAnimEncoderAdd(nativeint encoder, WebPPicture& picture, int timestampMilliseconds, WebPConfig& config)

    [<DllImport("libwebpmux", EntryPoint = "WebPAnimEncoderAdd", CallingConvention = CallingConvention.Cdecl)>]
    extern int WebPAnimEncoderFinish(nativeint encoder, nativeint picture, int timestampMilliseconds, nativeint config)
